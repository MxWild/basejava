        ...
        <c:forEach var="type" items="<%=SectionType.values()%>">
            ...
            <c:choose>
                <c:when test="${type == 'OBJECTIVE'}">
                    <input type="text" name="${type}" size="75" value="<%=((TextSection) section).getDescription()%>">
                </c:when>
                <c:when test="${type == 'PERSONAL'}">
                    <textarea name="${type}" cols="75" rows="5"><%=((TextSection) section).getDescription()%></textarea>
                </c:when>
                                    ...
                                    <dl>
                                        ...
                                        <dd>
                                            <input type="text" name="${type}${counter.index}startDate" size="10"
                                                   value="<%=DateUtil.format(pos.getDateStart())%>" placeholder="MM/YYYY"/>
                                        </dd>
                                    </dl>
                                <dl>
                                    <dt>Конечная дата:</dt>
